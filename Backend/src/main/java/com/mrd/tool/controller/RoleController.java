...
public class RoleController extends BaseController {
    ...
    public ResponseMessage createRole(final @Valid @RequestBody RoleCreateForm form) {
        try {
            ...
            if (invalidData != null) {
                ...
                responseMessage = new ResponseMessage(HttpStatus.BAD_REQUEST.value(),false, invalidData,
                ...
                if (existRole != null) {
                    return new ResponseMessage(HttpStatus.CONFLICT.value(),false, HttpStatus.CONFLICT.getReasonPhrase(),
                            new MessageContent("Role đã tồn tại"));
                } else {
                    Role roleCreate = roleService.createRole(form, username);
                    return new ResponseMessage(HttpStatus.CREATED.value(),true, HttpStatus.CREATED.toString(),
                            new MessageContent(roleCreate));
                }
            }
        } catch (Exception e) {
            LOGGER.error("Error to save role >>> " + e.toString());
            return new ResponseMessage(HttpStatus.INTERNAL_SERVER_ERROR.value(),false, HttpStatus.INTERNAL_SERVER_ERROR.toString(),
    ...
    public ResponseMessage updateRole(final @Valid @RequestBody RoleUpdateForm form) {
        try {
            ...
            if (invalidData != null) {
                return new ResponseMessage(HttpStatus.BAD_REQUEST.value(),false, invalidData,
                        new MessageContent(null));
            } else {
                //Role exist
                String username = super.getLoggedInUsername();
                ...
                if (existRole == null || (existRole.getIsDeleted() != null && existRole.getIsDeleted() == 1)) {
                    return new ResponseMessage(HttpStatus.NOT_FOUND.value(),false, HttpStatus.NOT_FOUND.getReasonPhrase(),
                            new MessageContent("Không tồn tại Role với roleCode " + roleCode));
                } else {
                    try {
                        Role updateRole = roleService.updateByRoleCode(form, username);
                        if (updateRole != null) {
                            return new ResponseMessage(HttpStatus.OK.value(),true, HttpStatus.OK.toString(),
                        ...
                        return new ResponseMessage(HttpStatus.NOT_MODIFIED.value(),false, HttpStatus.NOT_MODIFIED.getReasonPhrase(),
